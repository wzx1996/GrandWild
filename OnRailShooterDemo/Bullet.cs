...
namespace org.flamerat.OnRailShooterDemo {
    public class Bullet : GrandWild.Entity.TimerBasedEntity {
        ...
        private void _OnTimeTick(GrandWild.Timer timer) {
            if (BelongingScenes.Count <= 0) return;
            var targets = (BelongingScenes[0] as GameScene).Enemies;
            ...
            foreach(var target in targets.ToArray()) {
                if (target.TestIfHit(this)) {
                    target.TakeDamage(Damage);
                    IsDestroyed = true;
                    break;
                }
            }
            MoveFor(new GlmNet.vec3(0, 0, -Speed * (float)timer.TargetInteval.TotalSeconds));
        }
    }
}